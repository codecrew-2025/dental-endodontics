...
const PGDashboard = ({ brandTitleOverride }) => {
  ...
  // Load PG identity for topbar
  useEffect(() => {
    const storedPgId = localStorage.getItem('pgId');
    const storedPgName = localStorage.getItem('pgName');
    const storedPgEmail = localStorage.getItem('pgEmail') || user?.email || '';

    if (storedPgId) setPgId(storedPgId);
    else if (user?.Identity) setPgId(user.Identity);

    if (storedPgName) setPgName(storedPgName);
    else if (user?.name) setPgName(user.name);
  ...
  const getInitials = () => {
    const name = pgName || user?.name || 'PG';
    return name
      .split(' ')
      .filter(Boolean)
      .map((w) => w[0])
  ...
  return (
    <div className="chief-layout">
              ...
              <div className="profile-dropdown-menu">
                <div className="dropdown-header">
                  ...
                  <div className="dropdown-user-info">
                    <div className="dropdown-name">{pgName || user?.name || 'PG'}</div>
                    {pgId && <div className="dropdown-id">ID: {pgId}</div>}
                    <div className="dropdown-email">{pgEmail || user?.email || ''}</div>
                  </div>
                </div>
      ...
      <div className="chief-body">
        {isSideNavOpen && (
          <aside className="chief-sidenav" aria-label="PG navigation">
...
/* ======= MERGED: remote version (khubaib-remote/main) appended below ======= */
// PGDashboard.jsx
import React, { useMemo, useState, useEffect, useRef } from 'react';
import './ChiefDoctorDashboard.css';
import './PGDashboard.css';
...
const PGDashboard = () => {
  ...
  const populateFormWithPatientData = (patientData) => {
    ...
    setFormData({
      ...
      vitals: patientData.clinicalExam?.vitals || '',
      constitutionalSigns: patientData.clinicalExam?.constitutionalSigns || '',
    ...
    try {
      ...
      const patientData = {
        ...
        clinicalExam: {
          ...
          constitutionalSigns: formData.constitutionalSigns || '',
              ...
              <div className="profile-dropdown-menu">
                <div className="dropdown-header">
                  ...
                  <div className="dropdown-user-info">
                    ...
                    <div className="dropdown-email">{pgEmail || user?.email || ''}</div>
                    {pgDepartmentLabel && (
                      <div className="dropdown-dept">{formatDepartmentLabel(pgDepartmentLabel)}</div>
                    )}
                  </div>
                </div>
